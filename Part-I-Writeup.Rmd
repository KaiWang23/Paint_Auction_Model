...
data_clean = function(x, train.omit=T){
  ...
  x1 = x1 %>%
  mutate(shape_recode = ifelse(Shape == ""| Shape==0, "Not Available",
...
model1 = lm(logprice~1 + Surface + dealer + 
    origin_cat + diff_origin + type_intermed + engraved + mat_recode+dealer:origin_cat+dealer:diff_origin+
    paired + lrgfont + lands_sc + othgenre + discauth + othartist+ SurfaceNA+IntermNA+still_life, 
    data = x1)
summary(model1)
```
