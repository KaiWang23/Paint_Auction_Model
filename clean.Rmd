...
set.seed(12)
bart.valid=bart(x.train=x.train2.new,y.train=y.train2.new,x.test=x.test.valid,verbose=FALSE,ndpost = 5000)
set.seed(12)
...
```{r}
library(bartMachine)
train2 = rbind(paintings_train,paintings_test)
train2 = clean.new(train2, NA.omit = FALSE)
...
y.train2.new=train2$logprice


bartMachineCV(X = x.train2.new, y = y.train2.new, 
...
ci.test = calc_prediction_intervals(bart.test, x.test.new,pi_conf = 0.95, num_samples_per_data_point = 10000) %>% exp()
ci.train = calc_prediction_intervals(bart.test, x.train.new,pi_conf = 0.95, num_samples_per_data_point = 10000) %>% exp()
ci.valid = calc_prediction_intervals(bart.test, x.test.valid,pi_conf = 0.95, num_samples_per_data_point = 10000) %>% exp()

load("all_predictions.Rdata")
#train
predictions = cbind(all_prediction$y.train, ci.train) %>% as.data.frame()
colnames(predictions) = c("fit","lwr","upr")
save(predictions, file = "predict-train.Rdata")

#test
predictions = cbind(all_prediction$y.test, ci.test) %>% as.data.frame()
colnames(predictions) = c("fit","lwr","upr")
save(predictions, file = "predict-test.Rdata")

#valid
predictions = cbind(all_prediction$y.valid, ci.valid) %>% as.data.frame()
colnames(predictions) = c("fit","lwr","upr")
save(predictions, file = "predict-valid.Rdata")
