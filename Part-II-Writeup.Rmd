...
train = clean(paintings_train, NA.omit = TRUE)
test = clean(paintings_test, NA.omit = FALSE)
...
predictions = predict(xgb_tune, newdata = test[,-1]) %>% exp() %>% as.data.frame()
colnames(predictions) = c("fit")
save(predictions, file = "predict-test.Rdata")
...
#training RMSE
predictions.train = predict(xgb_tune, data = train[,-1]) %>% exp() %>% as.data.frame()
...
validation = clean(paintings_validation, NA.omit = FALSE)
predictions.valid = predict(xgb_tune, newdata = validation[,-1]) %>% exp() %>% as.data.frame()
