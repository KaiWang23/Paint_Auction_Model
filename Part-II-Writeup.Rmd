...
boost.logprice = gbm(logprice~., data=train, 
                distribution="gaussian",n.trees =20000,
                interaction.depth = 3)

pred = predict(boost.logprice,newdata=test,n.trees=20000,
                       type="response")
pred = exp(pred)
...
#training RMSE
predictions.train = predict(boost.logprice,data=train,n.trees=10000,
                       type="response") %>% exp() %>% as.data.frame()
colnames(predictions.train) = c("fit")
rmse(predictions.train$fit, exp(train$logprice))

load("raw_test.Rdata")
predictions = predict(boost.logprice, newdata = test, n.trees = 10000) %>% exp() %>% as.data.frame()
colnames(predictions) = c("fit")
rmse(predictions$fit, exp(test.raw$logprice))

load("paintings_validation.Rdata")
load("raw_valid.Rdata")
validation = clean.new(paintings_validation, NA.omit = FALSE)
predictions.valid = predict(boost.logprice, newdata = validation, n.trees = 10000) %>% exp() %>% as.data.frame()
colnames(predictions.valid) = c("fit")
rmse(predictions.valid$fit, exp(valid$logprice))
```
...
train.new=train%>%
  dplyr::select(position,dealer , year , diff_origin ,origin_author, origin_cat, endbuyer , 
    ...
    other, mat_recode, shape_recode, logprice)
...
predictions = predict(xgb_tune, newdata = test.new) %>% exp() %>% as.data.frame()
colnames(predictions) = c("fit")
save(predictions, file = "predict-test.Rdata")

save(xgb_tune, file = "xgb.Rdata")
```
...
#training RMSE
predictions.train = predict(xgb_tune, data = train) %>% exp() %>% as.data.frame()
colnames(predictions.train) = c("fit")
rmse(predictions.train$fit, exp(train$logprice))

load("raw_test.Rdata")
predictions = predict(xgb_tune, newdata = test) %>% exp() %>% as.data.frame()
colnames(predictions) = c("fit")
rmse(predictions$fit, exp(test.raw$logprice))
...
predictions.valid = predict(xgb_tune, newdata = validation) %>% exp() %>% as.data.frame()
colnames(predictions.valid) = c("fit")
rmse(predictions.valid$fit, exp(valid$logprice))
```
 ...
 * Secondly, we learned the importance of not overfitting test data. During the first part, we tried hard to get on top ranks on the leaderboard and ended up with test RMSE of around 1270 but train RMSE with more than 2000. It's clear that we overfitted the test data and we do not expect such a model will perform well on the validation data. 
