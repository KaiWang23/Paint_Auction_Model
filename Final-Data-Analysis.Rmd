...
test=paintings_train%>%
  select(-sale,-year,-lot,-price,-authorstyle,-author,-Height_in,-Width_in,-Diam_in,-Surface_Rect,-Surface_Rnd,-material,-materialCat,-subject,-count,-other,-winningbidder,-Interm,-authorstandard,-school_pntg,-winningbiddertype,-Shape)


factors=c("dealer","origin_author","origin_cat","diff_origin","mat","endbuyer","type_intermed","artistliving","engraved","original","prevcoll","othartist","paired","figures","finished","lrgfont","relig","landsALL","lands_sc","lands_elem","lands_figs","lands_ment","arch","mytho","peasant","othgenre","singlefig","portrait","still_life","discauth","history","allegory","pastorale")
...
model2$xlevels[["endbuyer"]] = union(model2$xlevels[["endbuyer"]], levels(test$endbuyer))
model2$xlevels[["Shape"]] = union(model2$xlevels[["Shape"]], levels(test$Shape))

predicitions = exp(predict(model2, newdata = test, estimator = "BMA", se.fit = TRUE, prediction = FALSE)) %>% as.data.frame() #need some modification
save(predictions, file="predict-test.Rdata") 
```
...
rmse(predicitions, paintings_train$logprice)
```
